...
- 将 OpenCode/Agent 的 LLM 请求切换到本地 Ollama。
...
需要 Node.js 20+、已安装的 MiniMax Design 和 Ollama。
...
本地桥只监听 `127.0.0.1`。未知模型路由和未配置生成路由均被拒绝，不会代理至任何远程服务。遥测、更新检查、账号等非模型接口不属于本覆盖层的模型路由范围，仍可能使用原应用接口。
