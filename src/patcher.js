...
const MCP_ALLOWLIST_MARKER = "/* H3_LOCAL_MCP_TOOL_ALLOWLIST_V4 */";
const PREVIOUS_MCP_ALLOWLIST_MARKER = "/* H3_LOCAL_MCP_TOOL_ALLOWLIST_V3 */";
const LEGACY_V2_MCP_ALLOWLIST_MARKER = "/* H3_LOCAL_MCP_TOOL_ALLOWLIST_V2 */";
...
function patchedMcpToolsSource(source) {
  ...
  if (source.includes(PREVIOUS_MCP_ALLOWLIST_MARKER) || source.includes(LEGACY_V2_MCP_ALLOWLIST_MARKER)) {
    const previousBlock = /\/\* H3_LOCAL_MCP_TOOL_ALLOWLIST_V[23] \*\/\s*const h3LocalToolAllowlistRaw = process\.env\.HILO_MCP_TOOL_ALLOWLIST\?\.trim\(\) \|\| [\s\S]*?;\s*const h3LocalToolAllowlist = new Set\([\s\S]*?;\s*const h3LocalToolAllowed = \(name\) => h3LocalToolAllowlist\.has\(name\);/;
    if (!previousBlock.test(source)) {
      throw new Error("Unsupported mcp-tools build: previous tool allowlist block was not found. No file was changed.");
    }
    return source.replace(previousBlock, MCP_ALLOWLIST_BLOCK);
  }
